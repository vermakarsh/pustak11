...
# Test the test donations API (without authentication)
url = "http://localhost:9001/api/donations/test"
...
try:
    ...
    if response.status_code == 200:
        ...
        if 'data' in data and len(data['data']) > 0:
            print("\n=== SAMPLE DONATIONS ===")
            for i, donation in enumerate(data['data'][:3]):  # Show first 3 donations
                print(f"\nDonation {i+1}:")
                ...
                print(f"  Condition: {donation.get('bookCondition')}")
                print(f"  Donor: {donation.get('donor', {}).get('name')}")
                print(f"  Created: {donation.get('createdAt')}")
        else:
            print("No donations found in response")
